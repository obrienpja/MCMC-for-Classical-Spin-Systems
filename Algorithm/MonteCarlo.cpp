...
void MonteCarlo::simulate(const int nitr, std::unique_ptr<Model> & model_ptr, \
double temp, std::vector<double> & average_energy_list) {
...
//  	output_json["data"].push_back(model_ptr->save_spin_configuration(accepted_counter).str());
    ...
    for (int i = 0; i < nitr; i++) {
        ...
        double change_eng(model_ptr->energy_change(random_index, lattice_site, \
        old_spin_vec, new_spin_vec));
...
//    print_energy_list();
    ...
    average_energy_list.emplace_back(tot_energy/(accepted_counter + 1)/model_ptr->get_system_size());
